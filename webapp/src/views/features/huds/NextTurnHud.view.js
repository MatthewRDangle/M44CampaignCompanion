        ...
        view: (vNode) => {
            ...
            return (
                m('div', {className: 'group absolute bottom-8 right-8 z-1 shadow-background-500/50 select-none'},
                    m('button', {
                        className: 'block w-16 h-16 p-3 pl-4 m-auto mb-2 bg-tertiary group-hover:bg-secondary border-2 border-solid border-secondary rounded-full cursor-pointer',
                    ...
                    m('span', {className: 'block px-4 py-1 text-sm text-font group-hover:text-background bg-background group-hover:bg-secondary rounded-full shadow-lg'}, attrs.label || 'Next Turn')
                )
            )
        }
    }
